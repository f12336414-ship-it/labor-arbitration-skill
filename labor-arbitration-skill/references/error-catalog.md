...
| `LEGAL_MONITOR_DEFINITION_*` / `LEGAL_MONITOR_SOURCE_*` | 监控定义、来源身份、允许列表、基线、间隔或错误预算无效 | 从已验证冻结记录重新建立唯一且排序的来源定义；不得手改定义 ID、快照或放宽官方来源边界 |
| `LEGAL_MONITOR_CHECK_*` / `LEGAL_MONITOR_FRESHNESS_*` | 本批检查重复、来源未知或没有准确绑定基线、发布者、检查时间和技术年龄 | 使用同一次运行时间和该来源已验证的冻结记录重新构建检查；不要跨来源或跨基线复用 |
| `LEGAL_MONITOR_PREVIOUS_*` / `LEGAL_MONITOR_TIME_ROLLBACK` | 直接前序缺失、损坏、来源集合不一致或运行时间回退 | 提供未改动的直接前序运行并使用不早于前序的 UTC 时间重建；保留完整不可变运行历史 |
| `LEGAL_MONITOR_RUN_*` | 运行 ID、快照、来源状态、告警或错误预算不等于确定性派生结果 | 用锁定定义、直接前序和本批检查重新生成；任何开放告警都保持 `DRAFT`，不得手改状态关闭告警 |
