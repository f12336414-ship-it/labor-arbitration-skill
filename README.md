...
[案件包 JSON Schema](labor-arbitration-skill/references/case-package.schema.json)与[intake manifest JSON Schema](labor-arbitration-skill/references/intake-manifest.schema.json)是 v1.3 结构契约；[审查包契约](labor-arbitration-skill/references/review-packet-contract.md)及其 [JSON Schema](labor-arbitration-skill/references/review-packet.schema.json)定义三类 v1.0 交叉验证对象；全部[合成示例](examples/review-packets/synthetic-rule-review.json)均不含真实案件数据或真实法律结论。
